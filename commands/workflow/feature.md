...
## 🚨 MANDATORY — Sub-Agent Spawn 鐵則（違反就是錯）

**每一次** 呼叫 `Agent(...)`（Step 1、2、3、4、4.5 全部包含），參數 MUST 含：

1. **`model="sonnet"`** — 不傳就會繼承 parent 的 Opus，token 成本至少 5×。**這是預設失敗，不是預設成功。**
2. **`subagent_type=...`** — 指定型別。Step 3 用 `"speckit-executor"`，其他用 `"general-purpose"`。
3. **Fallback 規則**：若 `speckit-executor` 在當前環境的 agent 列表中不存在，降級用 `subagent_type="general-purpose"`，**但 `model="sonnet"` 絕對不可省略**。

### Spawn 前的 self-check（每批都做，沒例外）
...
看到範例 code block 不代表你會自動套用 — 抄形狀也要抄參數。

---

## Step 0 — Worktree 隔離

在做任何事之前，先呼叫 `EnterWorktree` 建立隔離環境：

```
EnterWorktree(name="feature/{feature-slug}")
```

- `{feature-slug}` 從 `$ARGUMENTS` 取關鍵字，用 kebab-case（例如 `feature/add-dark-mode`）
- 這確保整個功能開發不影響原本的工作目錄
- 所有後續步驟（Step 1–5）都在 worktree 內執行
...
開一個 Agent（`subagent_type="general-purpose"`, `model="sonnet"`），給它以下指令：
...
你自己不寫任何實作程式碼。你的職責：解析 tasks → 分組 waves → spawn sub-agents（每個 task 以 TDD 循環開發）→ 收集結果 → 更新 tasks.md。
...
5. **確認測試指令**：檢查 repo 根目錄的 Makefile、package.json scripts、README，找到該 repo 的測試指令（例如 admin repo 是 `make test`）。不要假設，不要自己組 npx/jest 指令。將測試指令傳給每個 sub-agent。
...
### 3.3 逐 wave 執行（TDD 循環）

每個 sub-agent 必須以 **TDD 流程** 開發：先寫 failing test，再實作到 green。

**Sequential tasks** — 直接開 Agent（`subagent_type="speckit-executor"`，**fallback** 用 `"general-purpose"` 若前者不存在；無論如何都帶 `model="sonnet"`）：
...
<development_method>
TDD — 對每個 task 執行以下循環：
1. **Red** — 先寫 failing test，明確定義該 task 的預期行為
2. **Green** — 實作最少量程式碼讓測試通過
3. **Refactor** — 在測試保護下重構，保持 green
4. **Commit** — 每完成一個 task 就 commit

測試指令：{test_command}
i18n mock 注意：如果 spec 有 mock i18n（如 useTranslation: () => (key) => key），t() 回傳的是 key 不是 value，expectation 用 key。
</development_method>

<success_criteria>
- [ ] Each task has corresponding test(s) written BEFORE implementation
- [ ] All tests pass
...
Agent(
  ...
  model="sonnet",
  isolation="worktree",
  prompt="<task>...</task> <files_to_read>...</files_to_read> <tasks>{single task}</tasks> <development_method>TDD — ...</development_method>"
)
Agent(
  subagent_type="speckit-executor",
  model="sonnet",
  isolation="worktree",
  prompt="<task>...</task> <files_to_read>...</files_to_read> <tasks>{another task}</tasks> <development_method>TDD — ...</development_method>"
)
```
...
## Step 4 — Lint & Style Review

開一個 Agent（`model="sonnet"`），給它以下指令：

> 你是資深工程師。請依序執行 lint 和規範審查（測試已在 Step 3 TDD 流程中逐 task 驗證過）：
> 1. `Skill(skill="clean-ts")` — 審查 TypeScript 程式碼品質
>
> 如果是 SHOPLINE 專案（檢查 package.json 或 repo 名稱含 shopline），額外執行：
> 2. `Skill(skill="shopline-frontend-coding")`
> 3. `Skill(skill="shopline-e2e-attributes")`
>
> 完成後回報結果。
...
**如果有 UI 變更**，開一個 Agent（`model="sonnet"`），給它以下指令：
...
- worktree 路徑與 branch 名稱
- 實作摘要
- 改了哪些檔案
- 提示使用者：可用 `ExitWorktree(action="keep")` 保留 worktree，或回到原目錄後手動 merge

---

## 原則

- **每個 Agent() 都帶 `model="sonnet"`** — 詳見頂部 MANDATORY 區塊，這是這個 workflow 最容易漏的地方
- **一開始就用 EnterWorktree 建立隔離環境，整個流程在 worktree 內執行**
